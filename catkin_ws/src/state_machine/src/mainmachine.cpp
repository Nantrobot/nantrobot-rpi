...
ros::Publisher pubcom= nh.advertise<geometry_msg::Point32>("????", 1000);
ros::Publisher pubbras= nh.advertise<????>("????", 1000);
ros::Publisher pubpince= nh.advertise<????::???>("????", 1000);
ros::Publisher pubparasol=nh.advertise<????::???>("????", 1000);
...
void State200(State* S){
 ...
 //On commande le robot vers la dune
            ...
            while (!poseatteinte&&(ros::Time::now()-last_state)<10) {//tant qu'on est pas arrivé et qu'on a pas fait 10s
                    ...
                    poseatteinte=true;}//on indique qu'on est arrivé
...
void State300(State* S){
 ...
 cout<<"approche lente"<<endl;
   ...
   while (!poseatteinte&&(ros::Time::now()-last_state)<3){//tant que l'on est pas arrivé et durant au maximum 3s
        ...
        cmd.z=1;
         pubcom.publish(cmd);
         poseatteinte=true//aremplir si on veut une condition d'arret non temporelle
         if (robot detecte) {
